...
export default {
  ...
  mounted() {
    ...
    this.formEl = document.getElementById('listForm')
    this.$nuxt.$on('listCreate', () => {
  ...
  methods: {
    ...
    resetFrom() {
      this.$formulate.reset('listsForm')
    },
    resetValidation() {
      this.$formulate.resetValidation('listsForm')
    },
    handleCreate() {
      ...
      this.$axios
        .post('/admin/lists', data)
        .then((res) => {
          console.log(res)
          vm.$nuxt.$emit('refreshListsTable')
          vm.$toast.success('List created')
          vm.inputErrors = {}
          vm.resetFrom()
          // vm.resetValidation()
