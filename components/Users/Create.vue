      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
          ...
          <button type="button"
            ...
            data-modal-hide="staticModal" id="hideModalBtn">
        ...
        <div class="p-6 space-y-6">
          ...
          <div v-else>
            ...
            <FormulateForm @submit="handleSubmit" name="usersForm" id="userForm" :errors="inputErrors">
              <FormulateInput type="text" label="Name" name="name"
                ...
                v-model="user.name" validation="required" />
              <!-- email -->
              <FormulateInput type="email" label="Email" name="email"
                label-class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" outer-class="mb-3"
                input-class="input-style" error-class="text-red-700 text-xs mb-1" placeholder="xatta@email.com"
                v-model="user.email" validation="bail|required|email" />
              <!-- username -->
              <FormulateInput v-show="id != 0" type="text" label="Username" name="username"
                label-class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" outer-class="mb-3"
                input-class="input-style" error-class="text-red-700 text-xs mb-1" placeholder="xatta-trone"
                v-model="user.username" :validation="id == 0 ? '' : 'required'" />

              <!-- username -->
              <FormulateInput v-show="id != 0" type="date" label="Expires on" name="expires_on"
                label-class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" outer-class="mb-3"
                input-class="input-style" error-class="text-red-700 text-xs mb-1" placeholder="xatta-trone"
                v-model="user.expires_on" />
    ...
    getUserData(id) {
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          if (vm.user.expires_on != null) {
            vm.user.expires_on = (new Date(vm.user.expires_on)).toISOString().slice(0, 10)
          }
